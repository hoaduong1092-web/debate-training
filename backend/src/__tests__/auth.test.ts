...
export async function runAuthTests(): Promise<boolean> {
  ...
  section('TC-AUTH-02: HMAC-SHA256 OTP Generation & Verification');
  {
    ...
    const gen = OtpService.generateOtp(phone);
    ...
    const correctVerify = OtpService.verifyOtp(phone, gen.otp || '123456');
    assert('Verify with correct OTP succeeds', correctVerify.success === true);

    const reuseVerify = OtpService.verifyOtp(phone, gen.otp || '123456');
  ...
  section('TC-AUTH-03: 60-Second Request Cooldown');
  {
    ...
    const gen1 = OtpService.generateOtp(phone);
    assert('First OTP request succeeds', gen1.success === true);

    const gen2 = OtpService.generateOtp(phone);
  ...
  section('TC-AUTH-04: Max 5 Incorrect Attempts Lockout');
  {
    ...
    for (let i = 1; i <= 5; i++) {
      OtpService.verifyOtp(phone, `99999${i}`);
    }

    const verifyAfterLockout = OtpService.verifyOtp(phone, gen.otp || '123456');
  ...
  section('TC-AUTH-05: Max 5 Daily OTP Cap');
  {
    ...
    for (let i = 0; i < 5; i++) {
      // Clear lastRequestedAt to bypass 60s cooldown for test
      const r = OtpService.generateOtp(phone);
      // artificially adjust lastRequestedAt
      const storeMap = (OtpService as any).OTP_STORE;
    }
    // Check 6th request is blocked by daily cap
  }
